...
public class RssCategory {
  ...
  private ArrayList<Integer> randomizedIdArray; // Shuffled ID from the
  // Category.
  ...
  public static String[] getCategoryList(Context c) {
    String[] chars;
    final String[] proj = {
        Feed.Columns.CATEGORY
    }; // PROJECTION= only Category
  ...
  public static RssCategory loadLastFromPref(final Context c) {
    ...
    String name = pf.getString(Pref.LASTCATEGORY, Feed.Category.ALL); // LOAD
    // LAST
  ...
  // final String [] hist = history.array.toArray(new
  // String[history.array.size()]);
    ...
    try {
      ...
      do { // GET next from shuffled feed! WITH DELETEDS CHECK.
        ...
        if (history.getter >= randomizedIdArray.size() - 1) {
                    /*
                     * Checking if Getter is over the size of
                     * randomizedIdArray....
                     */
        ...
        String name = cur.getString(1); // 1 is Name
        String url = cur.getString(2); // 2 is URL
        rss = Feed.RssEntry.createRssFrom(c, name, url);
        cur.close();
        if (loopCheck >= randomizedIdArray.size()) {
                    /*
                     * LoopCheck. this loop happens if the user disable all the
                     * feeds in the category.
                     */
          Toast.makeText(c, c.getString(R.string.rsscategory_alldeletederror),
