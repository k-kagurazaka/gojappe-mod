...
public class MngAdapter extends CursorAdapter {
        ...
        builder.setItems(R.array.managermenu, new DialogInterface.OnClickListener() {
          public void onClick(DialogInterface dialog, int which) { // check
            ...
            if (which == 0) { // Zero is "OPEN in ReaderFragment"
              ...
              // a.startActivity(i);
              // a.finish();
              // RssEntry rss = RssEntry.createRssFrom(a, holder.name,
              // holder.url);
              // ReaderFragment rf =
              // ReaderFragment.newInstance("preview",rss);
      ...
      public void onClick(View v) {
        ...
        switch (which) {
          case fav: // -----> move to norm?
            v.setTag(normal);
            Feed.RssEntry.setStateNormal(a, holder.name);
            img.setImageResource(R.drawable.ic_menu_help);
            break;

          case deleted: // -----> move to FAV?
            v.setTag(fav);
            Feed.RssEntry.setStateFav(a, holder.name, holder.url);
            img.setImageResource(R.drawable.ic_menu_star);
            break;

          case normal: // -----> move to DEL?
            v.setTag(deleted);
            Feed.RssEntry.setStateDeleted(a, holder.name, holder.url);
  ...
  public View newView(Context c, Cursor arg1, ViewGroup parent) {
    View view = ((LayoutInflater) c.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(
        R.layout.bkmkitem, null);
  ...
  public void bindView(View view, final Context c, Cursor cur) {
    ...
    Spanned converted = Html.fromHtml(holder.category + "<font color=\"#FF6600\"> # </font>"
                                      + holder.name);
